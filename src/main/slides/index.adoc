...
most databases implement two types of latches, +
shared and exclusive (aka read and write)
...
when transaction is commited we replace +
...
Database system needs to translate a query into an executable query plan. But there are
different ways to execute each operator in a query plan (e.g., join algorithms, predicates orders) and there will be differences in performance among these plans.
...
[Concurrency Control in Transactions Processing Systems] +
